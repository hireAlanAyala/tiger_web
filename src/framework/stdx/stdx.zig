...
/// array_print provides the same comptime buffer-size proof but goes
/// through std.fmt.formatInt which handles padding, alignment, fill,
/// sign, and width (none used here). Benchmarked at ~100x slower on
/// ReleaseFast (5 ns/call vs <1 ns/call over 70M iterations).
...
///
/// Different from BoundedArrayType: BoundedList is a read-only result
...
// `man 2 fstatfs`. Linux-only — only called from io/linux.zig.
pub const TmpfsMagic = 0x01021994;
pub const StatFs = if (builtin.os.tag == .linux) extern struct {
...
pub fn fstatfs(fd: i32, statfs_buf: *StatFs) usize {
    if (builtin.os.tag != .linux) @compileError("fstatfs is Linux-only");
