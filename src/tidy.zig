...
test "tidy" {
    ...
    for (paths) |file_path| {
        // Principled skip — tiger-web-specific addition to TB's tidy.
        // `vendor/sqlite3/*` is 9MB+ third-party C; exceeds the 1MB
        // file_buffer and tidy's style/AST checks don't apply to
...
test "tidy changelog" {
    ...
    // Tracked follow-up — tiger-web hasn't shipped a CHANGELOG.md
    // yet (no public releases). When the first release lands and a
    // CHANGELOG is added, remove this guard and let the test run.
    // TB enforces CHANGELOG hygiene as part of their release flow.
    const changelog = SourceFile.read("CHANGELOG.md", changelog_buffer) catch |err| switch (err) {
...
test "tidy no large blobs" {
    ...
    while (lines.next()) |line| {
        ...
        if (std.mem.startsWith(u8, path, "kcov-out/")) continue;
        if (std.mem.startsWith(u8, path, "addons/shm/")) continue;
        if (std.mem.eql(u8, path, "packages/ts/native/shm.node")) continue;
...
test "tidy unix permissions" {
    const executable_files = [_][]const u8{
        ...
        // Tiger-web additions:
        "loadtest.sh",
        "scripts/cfo_supervisor.sh", // ours, not under src/
        "scripts/test_shm_protocol.sh",
        "packages/ts/src/bin/focus-codegen.ts",
        "packages/ts/src/bin/focus-sidecar.ts",
    };
    // Tiger-web addition — symlinks aren't in TB.
    const symlink_files = [_][]const u8{
        "examples/ecommerce-py/handlers/tiger_web.py",
    };
    ...
    while (lines.next()) |line| {
        ...
        if (std.mem.eql(u8, mode, "100644")) {
            ...
            // Principled — TB doesn't carry symlinks; tiger-web's
            // examples/ecommerce-py/ uses symlinks to share handler
            // code with adapters/python/ (pure Python files, single
            // source of truth). Allow mode 120000 for known symlinks.
            const expected_symlink = for (symlink_files) |sl| {
                if (std.mem.eql(u8, path, sl)) break true;
            } else false;
            if (!expected_symlink) return error.UnexpectedSymlink;
        } else {
...
test "tidy extensions" {
    const allowed_extensions = std.StaticStringMap(void).initComptime(.{
        ...
        .{".zig"},  .{".zon"},
        // Tiger-web additions:
        .{".sh"}, // shell scripts (zig/download.sh, scripts/*.sh, loadtest.sh)
        .{".sql"}, // storage/schema.sql, templates/ts/schema.sql
        .{".bin"}, // packages/vectors/*.bin (cross-language test vectors)
        // .dockerignore is a dotfile, not an extension — handled in exceptions below.
    });

    // Trimmed from TB's list to match our actual tracked files.
    // Each entry must correspond to a real file (the assertion loop
    // below panics if an exception doesn't exist).
    const exceptions = std.StaticStringMap(void).initComptime(.{
        // Common dot-files preserved from TB.
        .{".gitignore"},
        // Tiger-web specific (paths or basenames matching tracked files).
        .{"zig/download.sh"},
        .{"zig/download.ps1"},
        ...
        .{"Dockerfile"},
        .{".dockerignore"},
        // Per-example dot-files (intentional, no extension shape).
        .{".focus"},
        .{".nvmrc"},
        // Templates ship dot-prefixed files as `dot-X` to avoid
        // gitignore picking them up; renamed to `.X` on scaffold.
        .{"dot-focus"},
        .{"dot-gitignore"},
    });
