...
//!
//!     git clone https://github.com/hireAlanAyala/tiger_web && cd tiger_web
//!     while True:
//!         git fetch origin && git reset --hard origin/main
//!         zig build scripts -- cfo
//!
...
//! 3. pushes the new list to https://github.com/hireAlanAyala/tiger-web-devhubdb/
//!
//! Rules for merging:
//!
...
const Fuzzer = enum {
    ...
    // No per-fuzzer build variants, no -Drelease, no separate targets.
    ...
    fn args_run(comptime fuzzer: Fuzzer) []const []const u8 {
        return comptime .{"fuzz"} ++ .{"--"} ++ args_exec(fuzzer);
    }

    fn args_exec(comptime fuzzer: Fuzzer) []const []const u8 {
        // No "--" needed: TB's flags.zig falls through from named args to
        // positional parsing directly. The @"--": void in fuzz_tests.zig
        // CLIArgs is a struct-level delimiter, not a required CLI token.
        return comptime &.{@tagName(fuzzer)};
    }
...
pub fn main(shell: *Shell, gpa: std.mem.Allocator, cli_args: CLIArgs) !void {
    ...
    // Read-write token for <https://github.com/hireAlanAyala/tiger-web-devhubdb>.
    ...
    // Readonly token for PR metadata of <https://github.com/hireAlanAyala/tiger_web>.
...
fn run_fuzzers_prepare_tasks(tasks: *Tasks, shell: *Shell, gh_token: ?[]const u8) !void {
    ...
    for ([2]SeedRecord.Template.Branch{ .main, .release }) |branch| {
        ...
        for (std.enums.values(Fuzzer)) |fuzzer| {
            const fuzzer_present_on_branch = std.mem.indexOf(
                u8,
                branch_cfo,
            ...
            if (fuzzer_present_on_branch) {
                try tasks.put(working_directory, .{
                    ...
                    .branch_url = switch (branch) {
                        .main => "https://github.com/hireAlanAyala/tiger_web",
                        .release => "https://github.com/hireAlanAyala/tiger_web/tree/release",
                        else => unreachable,
                    },
                });
            }
                ...
                if (labeled or fuzzer == .canary) {
                    ...
                    try tasks.put(pr_directory, .{
                        ...
                        .branch_url = try shell.fmt(
                            "https://github.com/hireAlanAyala/tiger_web/pull/{d}",
                            .{pr.number},
                        ),
                    });
                }
...
fn run_fuzzers_prepare_repository(shell: *Shell, target: union(enum) {
    ...
    if (!try shell.dir_exists(".git")) {
        try shell.exec("git clone https://github.com/hireAlanAyala/tiger_web .", .{});
    }
...
const SeedRecord = struct {
    ...
    const Template = struct {
        ...
        const Branch = union(enum) {
            ...
            const main_url = "https://github.com/hireAlanAyala/tiger_web";
            const release_url = "https://github.com/hireAlanAyala/tiger_web/tree/release";
            const pull_url_prefix = "https://github.com/hireAlanAyala/tiger_web/pull/";
