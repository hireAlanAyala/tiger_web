...
pub fn ConnectionType(comptime IO: type, comptime options: Options) type {
    return struct {
        ...
        comptime {
            assert(frame_max == protocol.frame_max);
        ...
        fn send_now(self: *Self) void {
            while (self.send_queue.head_ptr()) |entry| {
                ...
                while (self.send_pos < entry.len) {
                    const n = self.io.send_now(self.fd, buf[self.send_pos..]) orelse return; // WouldBlock
                    if (n == 0) {
                        // send_now returns 0 on error (ECONNRESET, EPIPE).
                        // Use .shutdown to signal peer — unlike recv 0 (orderly
                        // close), send 0 means the connection is broken.
                        self.terminate(.shutdown);
                        return;
                    }
...
fn test_build_frame(buf: []u8, payload: []const u8) []const u8 {
    assert(payload.len <= protocol.frame_max);
...
const TestConnection = ConnectionType(TestIO, .{ .send_queue_max = 4 });
