...
- **When adding a new fuzzer, register it in CFO too.** Wiring `foo_fuzz.zig` into `src/fuzz_tests.zig` is half the job — the fuzzer also needs an entry in `src/scripts/cfo.zig`'s `Fuzzer` enum and weights table, or it gets zero continuous-fuzzing coverage and rides on smoke-seed luck alone. CFO calls `tiger-fuzz {tag_name}`, so enum tags must match `fuzz_tests.zig` Fuzzers exactly. The 2026-04-30 audit found four boundary fuzzers (`message_bus`, `worker_dispatch`, `codec`, `render`) registered in `fuzz_tests.zig` but missing from CFO entirely, plus a stale `sidecar` entry that referenced a non-existent fuzzer (silently failing every CFO dispatch attempt for an unknown duration). The TB-shape testing-layer model — smoke for sanity, CFO for swarm coverage, VOPR/sim for correctness — only works when each layer is actually running. Verify `cfo.zig`'s roster matches `fuzz_tests.zig` whenever you touch either.
