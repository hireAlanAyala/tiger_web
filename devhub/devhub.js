...
// divergence from TB's source is documented below; every line not listed
// is TB's code unchanged. Style preservation (snake_case, `deno fmt`, no
// TypeScript, no build step) is intentional — matches TB's choice.
//
// At the moment, it isn't clear what's the right style for this kind of
// non-Zig developer facing code, so the following is somewhat arbitrary:
//
// - snake_case naming
// - `deno fmt` for style
...
//
// **Removed vs TB:**
//   - `main_release_rotation()` + `get_release_manager()` helpers and
//     their `Promise.all` invocation. TB rotates a hardcoded roster of
//     team members as "this week's release manager"; tiger-web is a
//     solo project with no analog. `get_week()` helper retained — it's
//     dead code for now but kept as a free primitive in case a future
//     release-rotation lands.
//
// **Surgical edits vs TB (all URLs):**
//   - `tigerbeetle/devhubdb` → `hireAlanAyala/tiger-web-devhubdb` (fuzz +
//     metrics data URLs, logs base, tree-raw-data anchors).
//   - `tigerbeetle/tigerbeetle` → `hireAlanAyala/tiger_web` (issues API,
//     commit links, branch-identity check, release-tree check, PR prefix).
//
// **Preserved verbatim even when inert:**
//   - VOPR-related branches (`record.fuzzer === "vopr"`) — zero cost;
//     ready when/if a VOPR-analog ships (`docs/plans/vopr-analog-sketch.md`).
//   - `is_release()` — inert until we adopt `release/vN.N` tag convention.
//   - `outlier_score` red-highlighting — single most valuable metric
//     feature after raw charting; named here to protect against future
//     "tidy up" passes.
...
async function main_seeds() {
  const data_url =
    "https://raw.githubusercontent.com/hireAlanAyala/tiger-web-devhubdb/main/fuzzing/data.json";
  const issues_url =
    "https://api.github.com/repos/hireAlanAyala/tiger_web/issues?per_page=200";
  const logs_base =
    "https://raw.githubusercontent.com/hireAlanAyala/tiger-web-devhubdb/main/";
  ...
  for (const record of records) {
    ...
    row_dom.innerHTML = `
          <td>
            <a href="https://github.com/hireAlanAyala/tiger_web/commit/${record.commit_sha}"><code>${
...
function is_main(record) {
  return record.branch === "https://github.com/hireAlanAyala/tiger_web";
}
...
function pull_request_number(record) {
  const pr_prefix = "https://github.com/hireAlanAyala/tiger_web/pull/";
      ...
      chart: {
        ...
        events: {
          dataPointSelection: (event, chartContext, { dataPointIndex }) => {
            window.open(
              "https://github.com/hireAlanAyala/tiger_web/commit/" +
